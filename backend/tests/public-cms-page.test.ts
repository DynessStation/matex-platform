...
test('public lookup binds server tenant and slug, with publication boundary filters', async () => {
  ...
  for (const clause of [
    'p.cms_page_deleted_at IS NULL', 'p.cms_page_status = 1',
    'p.cms_page_visibility IN (1, 2)', 't.cms_page_i18n_status = 1',
    't.id_master_comp = p.id_master_comp',
    'p.cms_page_publish_at <= NOW()', 'p.cms_page_unpublish_at > NOW()',
...
test('unlisted response is noindex and strips internal fields and storage paths', async () => {
  reset();
  rows = [[{
    id_cms_page: 42, cms_page_visibility: 2, cms_page_locale: 'id-ID',
    cms_page_slug: 'about', cms_page_title: 'About', cms_page_content: '<p>Public</p>',
    cms_page_meta_robots: 'index, follow', cms_page_settings_json: 'private',
  }], [{ locale: 'id-ID', slug: 'about' }], [{
    role: 'hero', storage_path: 'cms/hero.png', name: 'Hero', alt: 'Alt', caption: 'Caption',
  }]];
  const response = await fetch(`${base}/id-ID/about`);
  ...
  for (const clause of ['c.cms_page_attachment_is_public = 1', 'a.attachment_status = 1', 'a.deleted_at IS NULL']) {
